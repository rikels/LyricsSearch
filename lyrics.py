	...
	if('search_result' not in locals()):
		...
		if isinstance(xml["return"]["fileinfo"], list):
			for item in xml["return"]["fileinfo"]:
				print("waarschijnlijk gaat het hier mis")
				...
				i += 1
				print("als je dit niet ziet ging het mis")
			...
			try:
				...
				title = None
			results.append({'artist': artist, 'title': title, 'rating': float(rating), 'filetype': xml["return"]["fileinfo"]["@link"].split(".")[-1], 'url': (server_url + xml["return"]["fileinfo"]["@link"])})
	return(results)
...
def LyricWikia(artist,title):
	...
	curl_url = u'http://lyrics.wikia.com/api.php?artist={artist}&song={title}&fmt=json'.format(artist=artist,title=title).replace(" ", "%20")
